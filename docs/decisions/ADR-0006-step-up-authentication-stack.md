...
## Amendment 2026-09-06 — interim adversarial review (docs/reviews/review-m2-interim-containers-and-step-up.md)

Findings R2-01 and R2-04 were DEFECTs in the decision as first shipped; R2-09/R2-10/R2-11 tightened it. Migration `20260906001200_step_up_hardening.sql` and the same-day worker/contract changes make the decision read as follows:

1. **Only the first passkey may be added with TOTP alone** (`REGISTER_PASSKEY` is `FIRST_PASSKEY_AAL2`, not `AAL2_ONLY`). `ops.begin_step_up` issues a first-passkey challenge only when the session's JWT `amr` claim shows a TOTP verification within the last 5 minutes (`ops.session_recent_totp`); an old aal2 session cannot add a credential. Every later registration needs a verified assertion from an existing passkey (`verifyPasskeyRegistration` requires `existingPasskeyVerdict.verified` when `activePasskeys > 0`).
2. **A first passkey cools for `FIRST_PASSKEY_COOLING_MS` (1 hour)** before it can authorise any `REQUIRED` kind (`ops.operator_passkeys.usable_from`; `verifyStepUp` returns `PASSKEY_COOLING`). Every registration raises a CRITICAL `OPERATOR_PASSKEY_REGISTERED` notification by database trigger, so a hijacked session adding a credential is visible before it becomes usable (out-of-app delivery arrives with M8a).
3. **Challenges are consumed atomically by the backend.** `ops.consume_step_up_challenge()` (service role only) marks the challenge consumed and inserts the immutable assertion in one call; a second caller gets `CHALLENGE_UNAVAILABLE`. The worker acts on a control request only after that call returns a row. `begin_step_up` no longer deletes rows, so an assertion referencing an expired challenge can never block an operator.
4. **The bound payload excludes the evidence keys** (`stepUp`, `registration`): `stepUpBindingHash` hashes `boundPayload(payload)`. The policy table is pinned into the contract digest as `policy.StepUpPolicy`. Every browser-supplied WebAuthn field is length-bounded and `stepUpAssertionRef` is a UUID.
5. **Accepted trade-off (R2-07):** an operator account that has not yet enrolled TOTP can be enrolled from a stolen aal1 session. Sign-up is disabled, so the window is between admin provisioning and first enrolment; provisioning and enrolment happen in one sitting (runbook), and the cooling period plus CRITICAL alert cover the next step. Forcing enrolment at first sign-in is scheduled with the §20.26 UI in M9.
