...
- [ ] **M1** Nx workspace, tags + boundaries, `/libs/contracts` complete, `ExecutionAdapter` + shared Jupiter client, digest harness, `Clock`, separated state machines with `UNRESOLVED(reason)` + property tests, signing primitives, **adversarial review #0**
- [ ] **M2** Supabase migrations + RLS, four `pgmq` queues, Auth/roles/TOTP/WebAuthn lib, Broadcast contracts, heartbeat/lease, hash-chained audit, config trust classes, **deployment profiles in config + CI**, web skeleton on Vercel, Docker profiles, GH Actions/GHCR, OTel/Sentry
