...
**Repo state at plan date (2026-09-05):** one blueprint file, no git, no code.
**Repo state now:** see §8; M0–M4 complete (M4 closed 2026-09-07), hosted Supabase carries 18 migrations with CLI history repaired 2026-09-08. Plan amended 2026-09-08 by ADR-0007 … ADR-0011 after the operator's review of v4 (`docs/reviews/review-plan-v4-external-recommendations-2026-09-07.md`).
...
**ADRs written in M0 (deviations from blueprint text)** — amended 2026-09-08 by **ADR-0007** (Profile 2 restricted capability set, `S0_LIVE_SAFETY` dependency, shared chain-foundation ownership), **ADR-0008** (signer-side policy mandatory for the Profile 2 shape set; Probe A split four ways; supersedes ADR-0004's "preferred, not required" row), **ADR-0009** (financial-boundary properties P1–P7 with owners), **ADR-0010** (applicability-based readiness, invariant wording fixes, bypass tests, deployment-bound evidence, severity wording), **ADR-0011** (freshness by strategy speed tier) and the ADR-0001 amendment (every unsuccessful open-position outcome normalises to protection; `BUDGET_PAUSED` equivalence proven; lot-scoped review deferred behind `SINGLE_SLEEVE_PER_MINT`):
...
- **Probe D (Trigger authentication, added 2026-09-08):** wallet-message authentication under the selected signer policy — approved challenge format and domain, challenge freshness and replay behaviour, a signing permission narrow enough that it cannot become arbitrary message signing; routing, signer and custody-release availability recorded as separate results.
- **Probe A is recorded as four results** (ADR-0008): deny-export/policy administration; Profile 2 shape acceptance; malicious-shape rejection; lookup-table compatibility.
- **Probe authorization boundary** (`docs/probes/README.md`): wallet value and cumulative fee/loss caps, allowed assets/programs/destinations, approved script and build hashes, who authorizes and runs, abort and recovery, post-probe reconciliation, credential cleanup. The cold-recovery destination is registered before the first probe that could need it (moved here from M8a).

**Exit gate:** all four probes' results recorded; consequences captured as ADRs (signer fallback: Privy with correlated-provider risk accepted, or an isolated signer-policy gateway; protection default: Trigger where Probe B passed, else `MONITORED_EXIT`). MP closes before M3 contracts harden and before M7.

### M3 — Financial boundary skeletons (XL) — *Track A*
Blueprint: P6 (partial), §35.7–9, §13.7, §15, D21/D22/D27/D45/D47/D49/D52/D55.
- **Consumes `/libs/solana-hard-state`** (the shared chain foundation, built and owned by M4 per ADR-0007: read-only RPC with a closed method set and a capability-boundary test, mint/token-account decoders, balance and security-state reads, slot/freshness metadata). M3 adds nothing to it except what the risk-authorizer's D45 reads need.
- **ADR-0009 owners in M3:** P3 (executor re-reads sticky `PAUSED`/arming immediately before submit; local pause), P4 (durable nonce/sequence journal, persist-before-submit, single-writer fencing token; restart, rollback and split-brain tests), P7 (execution-time bounds for every permitted shape, asserted against simulated deltas at landing). ADR-0008: the validator rejects every shape outside the Profile 2 set before signing.
...
- **Added 2026-09-08 (ADR-0007/0009/0011):** the candidate machine calls the self-influence guard (`libs/signals/src/self-influence`); canonical financial arithmetic module (P6: base-unit `bigint`, one rounding rule per operation, fee attribution and conservation identities as property tests) used by paper accounting; pending-exposure accounting (P1: authorized/submitted/provisional/reorg-pending counted as spent) in the paper book; an **explicit, reproducible latency/fill model** for paper fills (decision-time quote, modelled submission delay, executable price at that time, expiry/chase re-check, path-specific adverse-execution allowance defined once so it is not double-counted) — a stale decision price can never pose as a later executable fill; **paper/live parity suite** over identical inputs (sizing and rejection, expiry/chase, stop/trail transitions, lot allocation, retry/failure outcomes, fee and fill accounting) with the paper adapter's virtual balances and custody named as the only differences; freshness requirements bound to the Release's speed tier (ADR-0011).

**Exit gate:** **First end-to-end PAPER trades from live market data using S0 only, in an attended Profile 1A session**, each visible as an action cycle in the ledger. `STARTING` warm-up test green: no candidate can be scored before every enabled indicator's lookback history exists (D63). **Added:** one recorded `S0_SAFE` decision can be reconstructed from its stored inputs without consulting mutable current state; the parity suite is green; the fill model is documented and its parameters versioned.
...
**Exit gate:** P3 acceptance complete including the self-influence guard; S0 paper continues across the full trigger set. **Note (ADR-0007):** only the `S0_LIVE_SAFETY` subset — self-influence wiring, cohort inputs for enabled policy fields with unknown treated as most restrictive, speed/expiry/chase enforcement — is an M8a prerequisite; regime classification, correlation clusters, session labels and further trigger families are research breadth and do not gate tiny live.
...
- **ADR-0009 owners in M7:** P1 — the risk-authorizer refuses a second exposure-increasing authorization while one is in flight and counts pending exposure as spent (property test: N concurrent valid intents never exceed a cap; expiry never releases capacity that can still land); P2 — the state projector carries the audit sequence and hash of the `CLEARED` transition (proposal hash, cutoff, verdict, Release, lot) and the authorizer verifies the audit chain against the external checkpoint before authorizing (tamper tests: mutated cycle or audit row refused); P3 — authorizer re-reads sticky `PAUSED` and the arming record at authorization. `SINGLE_SLEEVE_PER_MINT` enforced at arming (ADR-0007).
...
### M8a — Tiny-live prerequisites (L) — *depends on M7 and on the `S0_LIVE_SAFETY` subset of M5b (ADR-0007)*
...
- **Added 2026-09-08 (ADR-0010):** `ReadinessRow` persisted per check, binding commit and image digest, contract digest, policy digests, wallet, cluster, profile, strategy Release, timestamp and expiry; any bound change invalidates the row. **Target-environment drills**, not only the harness: process termination after journal write; termination after submission before response persistence; runtime restart with an ambiguous transaction; DB outage with an open monitored position; signer outage; stale/divergent RPC; loss of operator presence; pending transactions during pause; local sleep/shutdown when the runtime is local. The minimum live UI shows pending and ambiguous executions, current protection state, why entries are blocked, unmanaged exposure and the actual outcome of each emergency command. **ADR-0009 P5:** protection lifecycle `INSTALLING → ACTIVE | FAILED → CANCELLING`; anything but `ACTIVE` is unprotected exposure for `WIND_DOWN` and readiness.

**Exit gate:** `READY_FOR_ATTENDED_TINY_LIVE` computable and green **for the `S0_SAFE` tiny-live variant**. Row set per ADR-0004 (required unless marked): risk-authorizer isolation and tamper tests; deny-export and pinned signer policy; persist-before-submit drill; approval binding and replay tests; tiny-live strategy variant bound to a `LIVE_APPROVAL` Release; wallet holds only tiny attested capital; reconciliation clean; `CRITICAL` out-of-app delivery and dead-man tested; out-of-band pause/close tested; break-glass revoke + sweep exercised on the probe wallet **and re-run on the trading wallet**; Probe C re-run on the trading wallet; signer-outage drill green; DB-down emergency close tested; wallet SOL/USDC reserve thresholds healthy; presence heartbeat; capital attestation; credentials in the isolated environment; artifact/egress/digest checks green; minimum live operator surface E2E green (approve, reject, arm, pause, mobile close, **Live Readiness FAIL blocks arming**); Trigger lifecycle test green **if** provider protection is enabled; **Probe A results (a) deny-export/policy administration, (b) Profile 2 shape acceptance and (c) malicious-shape rejection green — required (ADR-0008)**; `SINGLE_SLEEVE_PER_MINT` and the rest of the ADR-0007 capability set asserted at arming; **every invariant applicable to the Profile 2 capability set mapped and green, disabled capabilities reported `NOT_APPLICABLE` with reason (ADR-0010)**; **no unresolved critical/high security or financial-invariant finding**. **Adversarial review #3** (security + trading-correctness with emergency paths). Profile 2 may begin under `LIVE_APPROVAL` after this gate, for the ADR-0007 capability set only.
...
8. *After M10:* Strategy Lab (`S0_RAW` vs `S0_SAFE` distinct), **Replay Lab** (§20; run, compare and inspect replays with fidelity level and version ids visible), Trade History with export, Attribution/Economic P&L (three layers).
...
- **Added 2026-09-08:** model-weight look-ahead limits (a model trained after the replay window is labelled as such); forward holdout protocol; latency-matched baseline; proposer-only shadow versus proposer+adversary; a defined prediction target for confidence calibration.
...
- Invariant map: every invariant applicable to the capabilities enabled at Profile 3/4 mapped and green (ADR-0010); invariants for capabilities that remain disabled are `NOT_APPLICABLE` with a reason, not unmapped.
...
- 1A attended PAPER → 1B unattended all-session PAPER (cheapest measured runtime; collects Asia/Europe/US/weekend data). **May begin after M5b once M7's restart recovery exists**; downtime risks research data only, so notifications and the resume watchdog are not prerequisites. *Optional optimisation (operator review item 10, not required):* paper-book recovery over the shared position/reconciliation domain with a virtual custody adapter in M5b would let 1B start before M7; the trade-off is research delay versus implementing recovery once for both books.
...
- [x] **M4** Birdeye (REST → WS), OHLCV/snapshots + **retention jobs**, Price V3, eligibility on `solana-hard-state`, route probes + emergency route snapshots, held-asset safety, Helius reconciliation + tracked wallets, owned-address registry — done 2026-09-07 (packages a220cf9 → HEAD); evidence: `docs/probes/m4-exit-gate-2026-09-07.md` (every gate item with hosted-stack evidence: 9 ELIGIBLE of 104 assets on Birdeye Lite, candles HEALTHY, retention cron succeeded, safety/exit-compatibility and reconciliation roles live), interim adversarial review `docs/reviews/review-m4-interim-market-chain-eligibility.md` applied with a fix-status table (residuals deferred to M5a/M8), run-rate USD 39/month in `docs/costs.md`; migrations through 001800 on the hosted project; WebSocket deferred to a Premium tier (REST polling sized to the tier); Helius webhook receiver deferred to a hosted profile
- [x] **M5a** Core features, one trigger family, risk core (§13.1–13.5), `S0_SAFE`/`S0_RAW` **persisted as action cycles**, paper `ExecutionAdapter`, position monitor, minimal session lifecycle + **`STARTING` warm-up test**, thin operational UI, Level B capture, **first PAPER trades** — done 2026-09-08 (packages through `3bd8c5a`); evidence: `docs/probes/m5a-pipeline-evidence-2026-09-08.md` (first attended P1A paper trade 03:33 UTC: candidate → S0_RAW CLEARED / S0_SAFE REJECTED cycles → risk evaluation → intent → finalized paper fill → open MONITORED_EXIT lot, marked by the position monitor; STARTING warm-up observed on the hosted project; reconstruction re-run on the real decision and fill; parity table green; `paper-fill-v1` versioned); ADR-0007/0009/0011 additions landed (self-influence guard, base-unit arithmetic, pending exposure, fill model, parity suite, tier-bound freshness)
- [x] **M5b** Relative strength, regime, cohorts/clusters, remaining trigger families, **candidate self-influence guard**, speed tiers, session labels — done 2026-09-08 (packages `9b7b6e4` → HEAD); evidence: `docs/probes/m5b-evidence-2026-09-08.md` (every P3 deliverable and acceptance item mapped to code, tests and hosted observations; S0 versions 1.2.0 paper-trade both deterministic families; regime `regime-v1`, cohorts `cohorts-v1`, clusters `clusters-v1`, `features-v2` relative strength; D32 candidate-age expiry; suppression flag on feature snapshots); smart-money/catalyst/social families wait on tracked-wallet flow features and M6 intelligence
...
- [ ] **M7** State projector, sleeves/lots, signed envelope path on `S0_SAFE`, approvals + step-up, **tiny-live variant above reaction floor**, Releases + attestation, capital attestation, staged finality/REORG, restart recovery, manual actions — code complete 2026-09-08 (packages `36894a7` → `3a0ccce` and evidence commit); evidence: `docs/probes/m7-evidence-2026-09-08.md`; box stays open behind M3 (Turnkey adapter, review #1) and adversarial review #2
...
- [ ] **M8b** Direct-pool emergency adapters + dry-runs, executor emergency-route consumption *(parallel with M9)* — evidence: `docs/probes/m8b-evidence-2026-09-08.md` (adapters, dry-runs and executor consumption done 2026-09-08; Jito-style landing and the Profile 2 drill open)
- [x] **M9** §20 UI steps 1–7 and 9–10; step 8 after M10; wallet connector; mobile (full); accessibility; Playwright E2E *(parallel with M8b)* — done 2026-09-09: steps 1–7 and 9–10 landed 2026-09-08/09 (Control Room, Agent Activity / Inspector, Scanner / Watchlist / Asset Workspace, Approval Queue / arming / Readiness / Releases, Positions, Risk & Policy, Autonomy, Settings / Operator Security, Wallet / Custody, System Health, Alert Center, Audit Log, mobile, accessibility), wallet connector 6030803, step 8 research surfaces after M10 (Trade History with export, Strategy Lab, Replay Lab, Attribution / Economics); evidence: Playwright §24.7 suite `apps/web/e2e` (27 tests across authority, ledger, mobile and research specs; CI job `e2e`), `docs/probes/m10-evidence-2026-09-09.md` for the replay surfaces' data
- [x] **M10** Replay clock, fidelity levels, look-ahead enforcement, cost model, attribution, calibration, economic P&L, §30 queries — done 2026-09-09 (commit 9c54c62 and the follow-up digest fix); evidence: `docs/probes/m10-evidence-2026-09-09.md` (P9 acceptance: look-ahead test fails at guard, engine and live skill-source level; S0_RAW / S0_SAFE / latency-matched baseline decided the same 107-candidate timeline on the hosted Level B run `6c3fa605`; every result row carries strategy, variant and the run's version record; §30 views answer Q1, Q3–Q8, Q10, Q11, Q13–Q20 from the immutable replay record; Q2, Q21, Q22 wait for live catalyst-age and attendance rows)
- [ ] **M11** Drills automated, full SBOM tooling, Terraform written, invariant map complete, runbooks, **adversarial review #4**, Live Readiness verdicts, §33 DoD evidence — in progress: SBOM tooling and transitive forbidden-package policy landed 2026-09-09 (`tools/check-transitive.mjs`, CI step with planted negative test, CycloneDX 1.5 SBOM artifact per deployable); runbooks landed 2026-09-09 (`docs/runbooks/`: infrastructure loss chain-first recovery, wallet reserve and manual funding, signer outage, executor compromise); Terraform for P3/P4 written 2026-09-09 (`deploy/terraform`, unapplied per ADR-0002); automated drills landed 2026-09-09 (EXECUTE_READINESS_DRILL: CRITICAL_ALERT_DELIVERY, DB_DOWN_EMERGENCY_CLOSE, PERSIST_BEFORE_SUBMIT_DRILL run from Live Readiness; signer-outage and break-glass drills stay manual); remaining: invariant map completion (waits on Turnkey), adversarial review #4 (fresh session), Live Readiness verdicts (operator drills and probes), §33 DoD evidence
