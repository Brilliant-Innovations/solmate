...
| M4 (interim) | 2026-09-07 | Birdeye | Standard (free) | 0 | 30 000 CU/month, 1 rps, no token security (401). Measured with the M4 loops as configured: ≈1 000 CU/hour (ingest 90 CU per 30-min cycle, eligibility 40 CU per asset per 15-min cycle) → the month's allowance lasts ≈30 hours; the persisted ledger (`ops.provider_spend`) then refuses NORMAL work and keeps 10 % for CRITICAL held-asset safety. Lite (USD 39) is the lowest tier that unblocks eligibility; Premium (USD 199) adds the WebSocket stream. Decision pending. |
| M4 (interim) | 2026-09-07 | Helius | Free | 0 | 10 rps, 1 M credits/month. Serves the read-only RPC (`SOLANA_RPC_URL`, needed for `getTokenLargestAccounts`), Parsed Events for reconciliation and tracked wallets. One bot wallet consumed ≈150 history items per 2-minute poll; budget scales with tracked-wallet count and activity. |
| M4 (interim) | 2026-09-07 | Jupiter | Lite (free key) | 0 | Quote API for probes and safety sell quotes at 1 rps; Price V3 keyless. |
| M4 (interim) | 2026-09-07 | Supabase | Free | 0 | Database 16 MB after a day of M4 loops (market schema 752 kB, 2 421 one-minute candles for ≤10 tracked assets; 1 529 wallet events from one bot wallet in 12 minutes). Pro (USD 25) expected once the tracked universe or wallet stream grows; pg_cron retention is live. |
| **M4 interim total** | 2026-09-07 | | | **0** | Gate not declared: see `docs/probes/m4-exit-gate-2026-09-07.md`. Recommended next spend: Birdeye Lite USD 39/month. |
