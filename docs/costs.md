...
| M4 | 2026-09-07 | Birdeye | Lite (upgraded from Standard the same day) | 39 | 2.5 M CU/month, 15 rps, token security served. Standard (free) could not serve token security (401) and its 30 000 CU lasted ≈30 hours of M4 loops. On Lite as configured (ingest 5 min ≈90 CU, eligibility 10 min × 5 assets × 40 CU) ≈2 300 CU/hour ≈1.7 M CU/month; the persisted ledger (`ops.provider_spend`) keeps 10 % for CRITICAL held-asset safety. Premium (USD 199) adds the WebSocket stream when streaming is needed. |
...
| M9 / M10 / M11 (key-free items) | 2026-09-09 | none new | — | 39 | No tier changed: Birdeye Lite stays the only paid tier; GitHub Actions minutes grew with the e2e, SBOM and transitive-policy jobs but remain inside the free quota; Supabase Free holds the replay tables (three completed day-long Level B runs ≈ 60 decision rows + 50 trade rows each). Terraform for P3/P4 is written and unapplied (ADR-0002), so no host cost exists yet. |
