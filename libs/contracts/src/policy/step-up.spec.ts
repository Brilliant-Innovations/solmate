...
describe('D41 step-up policy', () => {
  ...
  it('keeps exactly the risk-reducing/neutral controls fast', () => {
    expect([...FAST_CONTROLS].sort()).toEqual(
      ['PAUSE_NEW_ENTRIES', 'REJECT_AUTHORIZATION', 'MANUAL_REDUCE', 'MANUAL_CLOSE', 'EMERGENCY_CLOSE_ALL', 'ACKNOWLEDGE_ALERT', 'RUN_READINESS_DRILL', 'END_SESSION', 'WATCH_ASSET', 'UNWATCH_ASSET', 'REQUEST_RESEARCH_REFRESH', 'FUND_TRADING_WALLET', 'RUN_REPLAY', 'EXECUTE_READINESS_DRILL'].sort(),
    );
