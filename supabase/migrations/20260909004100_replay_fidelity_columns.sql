...
alter table research.replay_trades add column fees_lamports bigint not null default 0 check (fees_lamports >= 0);
...
-- Attribution and economic rows are computed over the FULL variant only; saying so stops them
-- being read beside a leaderboard that splits by variant (review 2026-09-09, L-4).
