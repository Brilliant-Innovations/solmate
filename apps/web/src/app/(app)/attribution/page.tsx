...
export default async function Attribution({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <>
      ...
      <p className="muted">Period: last {days} days ({v.from.slice(0, 10)} → {v.to.slice(0, 10)}) · {[7, 30, 90].map((d) => <Link key={d} href={`/attribution?days=${d}`} style={{ marginRight: '0.5rem', fontWeight: d === days ? 700 : 400 }}>{d} d</Link>)} · closed lots only; paper and live labelled per row. Regime, session, liquidity and confidence attribution for replays live in the <Link href="/replay">Replay Lab</Link>.</p>
        ...
        {v.trading.length === 0 ? <p className="muted">No closed lot in the period.</p> : (
          <div style={{ overflowX: 'auto' }}>
            <table className="mono" style={{ borderCollapse: 'collapse' }}>
              ...
              <tbody>{v.trading.map((t) => <tr key={`${t.strategyVersionId}|${t.book}`}><td style={cell}>{t.strategyVersionId}</td><td style={cell}><span className="chip" data-tone={t.book === 'LIVE' ? 'live-auto' : 'paper'}>{t.book}</span></td><td style={cell}>{t.trades}</td><td style={cell}>{t.profitable}</td><td style={cell}>{num(t.grossUsdc)} USDC</td><td style={cell}>{num(t.routerFeesUsdc, 3)}</td><td style={cell}>{num(t.transferFeesUsdc, 3)}</td><td style={cell}>{((t.networkLamports + t.priorityLamports) / 1e9).toFixed(5)} SOL{t.networkPriorityUsd !== null ? ` (${usd(t.networkPriorityUsd)})` : ' (SOL price unknown)'}</td><td style={cell}>{num(t.slippageUsdc, 3)}</td><td style={cell}><span className="chip" data-tone={t.netUsdc > 0 ? 'ok' : t.netUsdc < 0 ? 'failed' : 'unknown'}>{num(t.netUsdc)} USDC</span></td></tr>)}</tbody>
            </table>
          </div>
        )}
      ...
      <section className="panel">
        ...
        {v.strategy.length === 0 ? <p className="muted">No closed lot in the period.</p> : (
          <table className="mono" style={{ borderCollapse: 'collapse' }}>
            ...
            <tbody>{v.strategy.map((s) => <tr key={`${s.strategyVersionId}|${s.book}`}><td style={cell}>{s.strategyVersionId}</td><td style={cell}>{s.book}</td><td style={cell}>{usd(s.netTradingUsd)}</td><td style={cell}>{usd(s.modelUsd)} ({s.modelRuns} run(s))</td><td style={cell}>{usd(s.dataRpcShareUsd)}</td><td style={cell}><span className="chip" data-tone={s.contributionUsd > 0 ? 'ok' : s.contributionUsd < 0 ? 'failed' : 'unknown'}>{usd(s.contributionUsd)}</span></td></tr>)}</tbody>
          </table>
        )}
        <p className="muted" style={{ margin: '0.4rem 0 0' }}>LLM cost is the ledger&apos;s <span className="mono">agents.runs.cost_usd</span> joined to each run&apos;s action cycle and strategy; the data/RPC share is metered Birdeye compute units at the Lite rate, allocated by turnover. USDC is treated as USD.</p>
      </section>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(28rem, 1fr))', gap: '1rem', alignItems: 'start' }}>
        <section className="panel">
          ...
          <table className="mono" style={{ borderCollapse: 'collapse' }}><tbody>
            ...
            <tr><td style={cell} className="muted"><strong>final operating result</strong></td><td style={cell}><span className="chip" data-tone={v.platform.finalOperatingResultUsd > 0 ? 'ok' : 'failed'}>{usd(v.platform.finalOperatingResultUsd)}</span></td></tr>
          </tbody></table>
          ...
          {v.platform.providerSpend.length > 0 ? <p className="muted" style={{ margin: '0.4rem 0 0' }}>Metered: {v.platform.providerSpend.map((p) => `${p.provider} ${p.month} ${p.usedCu.toLocaleString()} CU (${usd(p.estimatedUsd)})`).join(' · ')}</p> : null}
