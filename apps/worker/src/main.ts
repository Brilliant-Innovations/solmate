...
async function marketIngestLoop(env: WorkerEnv, logger: Logger, shared: SharedWithBirdeye, tier: (typeof BIRDEYE_TIERS)[keyof typeof BIRDEYE_TIERS]): Promise<void> {
  ...
  const deps = {
    ...
    config: { trackedLimit: tier.requestsPerSecond <= 1 ? 10 : 100, lookbackBuckets: LOOKBACK_BUCKETS, discoveryIntervalMs: Math.max(300_000, intervalMs), cuBudgetPerCycle, requestBudgetPerCycle, backoffBaseMs: Math.max(intervalMs, 300_000), backoffMaxMs: 6 * 3_600_000, evictAfterEmptyFetches: 5 },
  };
...
async function replayLoop(env: WorkerEnv, logger: Logger, shared: Shared): Promise<void> {
  ...
  const deps: ReplayRoleDeps = {
    ...
    directCosts: (run) => modelCostByStrategyBetween(sql, run.window.from, run.window.to),
