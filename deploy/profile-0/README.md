...
4. Start it. Container route: `docker compose up --build worker` from this directory. Process route: `pnpm nx serve @sol-agent-trader/worker` with the values exported in the shell. A successful start logs `startup`, then `market_ingest_starting` with the budgets computed from the tier, then one `market_ingest_cycle` line per cycle with counts of candles written and rejected, assets discovered, snapshots and errors. Feed health lands in `ops.provider_health` and is visible to the web app.
   ...
   Add `operator-security` once `WEBAUTHN_RP_ID` and `WEBAUTHN_ORIGINS` name the web app host: the role verifies passkey step-up evidence on every control request and applies passkey registration/revocation filed from Settings (§20.26). Without it, REQUIRED controls (resume, approve, promote, arm, revoke) stay refused as STEP_UP_REQUIRED.
   Add `watchlist` for the Scanner / Watchlist attention controls (WATCH_ASSET, UNWATCH_ASSET, REQUEST_RESEARCH_REFRESH; §20.4, §20.27). The `watchlist` role needs no keys.
   Add `funding` for the Wallet / Custody funding connector (§20.18): the web app builds and the operator's own Wallet Standard wallet signs a transfer into the trading wallet after the typed funding guard, then files a `FUND_TRADING_WALLET` report that this role records as a SUBMITTED `ops.wallet_funding_events` row; `reconciliation` confirms it from chain deltas. Needs no keys. Set `NEXT_PUBLIC_SOLANA_RPC_URL` on the web deployment if the public cluster RPC is too slow for balance reads.
   Add `replay` for the replay and research framework (§18, P9): file a run with `node tools/replay.mjs --env deploy/profile-0/.env.worker --name "S0 gate value" --from <iso> --to <iso>` (Level B by default; `--fidelity A_HISTORICAL` for candle-only), list runs with `--list`; the role binds every version and the dataset cutoff at filing, executes one run at a time and stores decisions, trades, digests and the §19 results in `research.replay_*`. Needs no keys.
